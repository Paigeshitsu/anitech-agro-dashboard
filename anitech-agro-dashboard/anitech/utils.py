...
CROP_TRANSLATIONS = {
    ...
    'Bottle Gourd': {'en': 'Bottle Gourd', 'tl': 'Upo'},
    'Snake Gourd': {'en': 'Snake Gourd', 'tl': 'Patola'},
    'Luffa': {'en': 'Luffa', 'tl': 'Lufa'},
    'Gabi': {'en': 'Taro', 'tl': 'Gabi'},
    'Cassava': {'en': 'Cassava', 'tl': 'Kamoteng Kahoy'},
    'String Beans': {'en': 'String Beans', 'tl': 'Sitaw'},
    'Bean': {'en': 'String Beans', 'tl': 'Sitaw'},
    'String Bean': {'en': 'String Beans', 'tl': 'Sitaw'},
    'Red Onion': {'en': 'Red Onion', 'tl': 'Pulang Sibuyas'},
    'Commercial Milled Rice': {'en': 'Commercial Milled Rice', 'tl': 'Bigas'},
    'Ampalaya': {'en': 'Bitter Gourd', 'tl': 'Ampalaya'},
    'Talong': {'en': 'Eggplant', 'tl': 'Talong'},
    'Kamatis': {'en': 'Tomato', 'tl': 'Kamatis'},
    'Pipino': {'en': 'Cucumber', 'tl': 'Pipino'},
    'Kalabasa': {'en': 'Squash', 'tl': 'Kalabasa'},
    'Sibuyas': {'en': 'Onion', 'tl': 'Sibuyas'},
    'Bawang': {'en': 'Garlic', 'tl': 'Bawang'},
    'Repolyo': {'en': 'Cabbage', 'tl': 'Repolyo'},
    'Patatas': {'en': 'Potato', 'tl': 'Patatas'},
    'Kamote': {'en': 'Sweet Potato', 'tl': 'Kamote'},
    'Mais': {'en': 'Corn', 'tl': 'Mais'},
    'Palay': {'en': 'Rice', 'tl': 'Palay'},
    'Bigas': {'en': 'Commercial Milled Rice', 'tl': 'Bigas'},
    'Pulang Sibuyas': {'en': 'Red Onion', 'tl': 'Pulang Sibuyas'},
}
...
LOWERCASE_CROP_TRANSLATIONS = {key.lower(): value for key, value in CROP_TRANSLATIONS.items()}
LOWERCASE_CROP_ALIASES = {key.lower(): value for key, value in CROP_NAME_ALIASES.items()}


def _format_crop_label(value):
...
def _normalize_crop_name(crop_name: Optional[str]):
    ...
    alias_match = LOWERCASE_CROP_ALIASES.get(raw_name.lower())
    normalized_name = alias_match or raw_name
    return _format_crop_label(normalized_name)
...
def get_crop_name(crop_name, lang='en'):
    ...
    """
    normalized_name = _normalize_crop_name(crop_name)
    translations = (
        CROP_TRANSLATIONS.get(normalized_name)
        or LOWERCASE_CROP_TRANSLATIONS.get(normalized_name.lower(), {})
    )
    if translations:
        translated_name = translations.get(lang, translations.get('en', normalized_name))
        return _format_crop_label(translated_name)
    return _format_crop_label(normalized_name or crop_name)
...
def get_current_lang(request):
    ...
    if session_lang in ['en', 'tl']:
        return session_lang

    user = getattr(request, 'user', None)
    user_language = getattr(user, 'language', '') if getattr(user, 'is_authenticated', False) else ''
    if str(user_language).lower() in ['tagalog', 'tl']:
        request.session['lang'] = 'tl'
        request.session['language'] = 'Tagalog'
        return 'tl'

    request.session.setdefault('lang', 'en')
    request.session.setdefault('language', 'English')
    return 'en'
...
def set_language(request, lang):
    ...
    """
    request.session['lang'] = 'tl' if lang == 'tl' else 'en'
    request.session['language'] = 'Tagalog' if request.session['lang'] == 'tl' else 'English'

    user = getattr(request, 'user', None)
    if getattr(user, 'is_authenticated', False):
        preferred_language = request.session['language']
        if user.language != preferred_language:
