...
LEGAZPI_LATITUDE = 13.1431
LEGAZPI_LONGITUDE = 123.7438
LEGAZPI_LOCATION_NAME = 'Legazpi City, Albay, Philippines'
LEGAZPI_MODEL_LOCATION = 'Bicol'
TRACE_RAINFALL_THRESHOLD_MM = 0.5


def _normalize_rainfall_amount(value):
    """
    Treat tiny trace amounts as no meaningful rainfall for UI/chart purposes.
    Open-Meteo can return very small non-zero values that users generally do
    not interpret as "it rained".
    """
    try:
        rainfall = float(value or 0)
    except (TypeError, ValueError):
        return 0.0

    return 0.0 if abs(rainfall) < TRACE_RAINFALL_THRESHOLD_MM else rainfall
...
def get_fallback_predictions(data):
    ...
    """
    from .model import (
        _curate_prediction_category,
        _get_market_demand_leaders,
        _normalize_recommendation_crop_name,
    )
    ...
    if requested_crops:
        ...
        seasonal_crops = {
            'Wet': ['Rice', 'Corn', 'Tomato', 'Eggplant', 'Cabbage', 'Onion', 'Squash', 'Banana', 'Coconut', 'Gabi', 'Kangkong', 'Pechay', 'Pole Sitao'],
            'Dry': ['Garlic', 'Sweet Potato', 'Cassava', 'Peanut', 'Bean', 'Chili', 'Watermelon', 'Mango', 'Bell Pepper', 'String Bean', 'Ampalaya', 'Upo']
        }
        crops = list(dict.fromkeys(
            normalized_name
            for normalized_name in (
                _normalize_recommendation_crop_name(crop_name)
                for crop_name in seasonal_crops.get(season, seasonal_crops['Wet'])
            )
            if normalized_name
        ))
    ...
    base_prices = {
        ...
        'Peanut': 140, 'Chili': 90, 'Squash': 28, 'Bean': 65, 'Beans': 80,
        'Cassava': 30, 'Mung Bean': 95, 'String Bean': 80, 'Carrot': 85,
        'Potato': 65, 'Calabaza': 45, 'Malunggay': 120, 'Kangkong': 60,
        'Sitaw': 70, 'Ampalaya': 55, 'Upo': 40, 'Banana': 45,
        'Coconut': 25, 'Gabi': 35, 'Pechay': 55, 'Pole Sitao': 70,
        'Watermelon': 55, 'Mango': 85, 'Bell Pepper': 95
    }
    ...
    crop_suitability = {
        ...
        'Beans': {
            'ideal_temp': (22, 30), 'ideal_rainfall': (80, 160), 'ideal_ph': (5.5, 6.8),
            'temp_tolerance': 4, 'rain_tolerance': 35, 'season_preference': {'Wet': 0.95, 'Dry': 1.1}
        },
        'Chili': {            'ideal_temp': (20, 28), 'ideal_rainfall': (80, 140), 'ideal_ph': (6.0, 6.8),            'temp_tolerance': 4, 'rain_tolerance': 30, 'season_preference': {'Wet': 0.9, 'Dry': 1.05}        },        'Banana': {            'ideal_temp': (25, 30), 'ideal_rainfall': (150, 300), 'ideal_ph': (5.5, 7.0),            'temp_tolerance': 4, 'rain_tolerance': 50, 'season_preference': {'Wet': 1.1, 'Dry': 0.9}        },        'Coconut': {            'ideal_temp': (25, 32), 'ideal_rainfall': (150, 250), 'ideal_ph': (5.5, 7.0),            'temp_tolerance': 4, 'rain_tolerance': 50, 'season_preference': {'Wet': 1.0, 'Dry': 1.0}        },        'Gabi': {            'ideal_temp': (24, 30), 'ideal_rainfall': (120, 200), 'ideal_ph': (5.5, 6.5),            'temp_tolerance': 4, 'rain_tolerance': 40, 'season_preference': {'Wet': 1.1, 'Dry': 0.9}        },        'Kangkong': {            'ideal_temp': (24, 32), 'ideal_rainfall': (100, 200), 'ideal_ph': (5.5, 7.0),            'temp_tolerance': 5, 'rain_tolerance': 45, 'season_preference': {'Wet': 1.1, 'Dry': 0.9}        },        'Pechay': {            'ideal_temp': (15, 22), 'ideal_rainfall': (80, 150), 'ideal_ph': (6.0, 7.0),            'temp_tolerance': 5, 'rain_tolerance': 30, 'season_preference': {'Wet': 1.0, 'Dry': 1.0}        },        'Pole Sitao': {            'ideal_temp': (22, 28), 'ideal_rainfall': (100, 180), 'ideal_ph': (5.5, 6.8),            'temp_tolerance': 4, 'rain_tolerance': 35, 'season_preference': {'Wet': 1.0, 'Dry': 1.0}        },        'Watermelon': {            'ideal_temp': (24, 32), 'ideal_rainfall': (60, 120), 'ideal_ph': (5.5, 7.0),            'temp_tolerance': 5, 'rain_tolerance': 25, 'season_preference': {'Wet': 0.85, 'Dry': 1.15}        },        'Mango': {            'ideal_temp': (24, 30), 'ideal_rainfall': (80, 150), 'ideal_ph': (5.5, 7.0),            'temp_tolerance': 4, 'rain_tolerance': 40, 'season_preference': {'Wet': 0.9, 'Dry': 1.1}        },        'Bell Pepper': {
            'ideal_temp': (20, 28), 'ideal_rainfall': (80, 150), 'ideal_ph': (6.0, 6.8),
            'temp_tolerance': 5, 'rain_tolerance': 30, 'season_preference': {'Wet': 0.9, 'Dry': 1.1}
        },
    }

    # Adjust scores based on environmental conditions
    demand_leaders = _get_market_demand_leaders(data.get('market_prices') or {}, crops)
    predictions = []
    for crop in crops:
        ...
        category = _curate_prediction_category(crop, season, final_score, demand_leaders)
...
def fetch_historical_weather_data(latitude=LEGAZPI_LATITUDE, longitude=LEGAZPI_LONGITUDE, days_back=7, timezone="Asia/Manila"):
    """
    Fetch historical weather data from Open-Meteo API.

    Args:
        latitude: Fixed Legazpi City latitude
        longitude: Fixed Legazpi City longitude
    ...
    try:
        ...
        # Calculate date range in Manila time. Open-Meteo archive data has about
        # a 5-day delay, so "7 days ago" on April 25, 2026 should end on
        # April 20, 2026 in Asia/Manila, not the server timezone.
        target_tz = pytz.timezone(timezone)
        current_date = datetime.now(target_tz).date()
        end_date = current_date - timedelta(days=5)
        lookback_days = max(1, int(days_back))
        start_date = end_date - timedelta(days=lookback_days - 1)

        latitude = LEGAZPI_LATITUDE
        longitude = LEGAZPI_LONGITUDE
        ...
        params = {
            ...
            "daily": "temperature_2m_max,temperature_2m_min,precipitation_sum,rain_sum,relative_humidity_2m_mean,relative_humidity_2m_max,wind_speed_10m_max",
            "timezone": timezone,
            "cell_selection": "land",
        }
...
def fetch_weather_data(latitude=LEGAZPI_LATITUDE, longitude=LEGAZPI_LONGITUDE, timezone="Asia/Manila"):
    """
...
def get_current_weather(latitude=LEGAZPI_LATITUDE, longitude=LEGAZPI_LONGITUDE):
    """
    ...
    try:
        ...
        weather_data = {
             ...
             'precipitation': _normalize_rainfall_amount(current.get('precipitation', 0)),
             'rainfall': _normalize_rainfall_amount(daily.get('rain_sum', [0])[0] if daily.get('rain_sum') else 0),  # Live daily rainfall in mm
...
def get_weekly_forecast(latitude=LEGAZPI_LATITUDE, longitude=LEGAZPI_LONGITUDE):
    """
    ...
    try:
        ...
        for i in range(min(7, len(times))):
            ...
            forecast.append({
                ...
                'rain_sum': round(_normalize_rainfall_amount(rain_sum[i]), 1) if i < len(rain_sum) else 0,
                'precipitation_sum': round(_normalize_rainfall_amount(precipitation_sum[i]), 1) if i < len(precipitation_sum) else 0,
                'humidity_mean': round(humidity_mean[i]) if i < len(humidity_mean) else 0,
                'wind_speed_max': round(wind_speed_max[i]) if i < len(wind_speed_max) else 0,
                ...
                'precipitation': _normalize_rainfall_amount(precipitation_sum[i]) if i < len(precipitation_sum) else 0,
                'hourly': hourly_data
            })
...
def get_fallback_forecast():
    ...
    for i in range(7):
        ...
        forecast.append({
            ...
            'rain_sum': 10 * (i % 3),
            'precipitation_sum': 10 * (i % 3),
            'humidity_mean': 65 + i,
            'wind_speed_max': 8 + (i % 4),
            'sunrise': '06:00',
            'sunset': '18:00',
            'icon': icons[i],
            'condition': 'Partly Cloudy',
...
def get_model():
    ...
    """
    # Temporarily disable ML model loading to prevent errors
    print("ML model loading disabled, using fallback predictions")
    return None
    ...
    #                 print("Model requires image input, skipping...")
    #             continue
    #
    # print("No valid ML models found, using fallback predictions")
    # return None
...
def weather_history(request):
    ...
    try:
        ...
        # Create cache key
        cache_key = f"weather_history_{WEATHER_CACHE_VERSION}_{period}_{latitude}_{longitude}"
        cached_data = cache.get(cache_key)
        if cached_data:
            return JsonResponse({
                'status': 'success',
        ...
        if config.get('forecast', False):
            ...
            chart_data = {
                ...
                'rainfall': [_normalize_rainfall_amount(value) for value in rain_sum],
                ...
                'hourly': {
                    ...
                    'rainfall': [_normalize_rainfall_amount(value) for value in hourly_rain]
                }
            }
        else:
            ...
            chart_data = {
                ...
                'temperature_min': temp_min,
                'rainfall': [
                    _normalize_rainfall_amount(value)
                    for value in (rain_sum if rain_sum else daily.get('precipitation_sum', []))
                ],
                'humidity_mean': humidity_mean,
...
def crop_care_recommendations(request):
    ...
    try:
        ...
        crop_name = data.get('crop_name', '')
        season = data.get('season', 'Wet')
        location = LEGAZPI_LOCATION_NAME
...
def ml_weather_trends(request):
    ...
    try:
        ...
        # Create cache key
        cache_key = f"ml_weather_trends_{WEATHER_CACHE_VERSION}_{latitude}_{longitude}_{period}"
        cached_data = cache.get(cache_key)
        if cached_data:
        ...
        if start_offset < 0:
            ...
            # For future periods, use the direct Open-Meteo daily/hourly response
            # so the charts reflect the actual forecast values without synthetic
            # reshaping.
            weather_result = fetch_weather_data(latitude, longitude)
            if not weather_result.get('success'):
                return JsonResponse({'error': 'Could not fetch weather data'}, status=503)
            chart_data = _build_openmeteo_forecast_chart_data(weather_result['data'], crop_name, period)
...
def _build_openmeteo_forecast_chart_data(weather_data, crop_name, period='7_days_forecast'):
    """
    Build chart payloads directly from Open-Meteo daily/hourly forecast data.
    This keeps the forecast charts aligned with the exact API response.
    """
    ...
    humidity_mean = daily.get('relative_humidity_2m_mean', [])[:7]
    wind_speed_max = daily.get('wind_speed_10m_max', [])[:7]

    if not dates:
        return _generate_fallback_chart_data()

    trend_indicators = _calculate_trend_indicators(temps_max, rainfall, humidity_mean)

    return {
        'period': period,
        'dates': dates,
        'temperature_max': [round(float(t), 1) for t in temps_max],
        'temperature_min': [round(float(t), 1) for t in temps_min],
        'rainfall': [round(_normalize_rainfall_amount(r), 1) for r in rainfall],
        'humidity_mean': [round(float(h), 1) for h in humidity_mean],
        'wind_speed_max': [round(float(w), 1) for w in wind_speed_max],
        'hourly': {
            'time': hourly.get('time', []),
            'temperature': hourly.get('temperature_2m', []),
            'humidity': hourly.get('relative_humidity_2m', []),
            'wind_speed': hourly.get('wind_speed_10m', []),
            'rainfall': [_normalize_rainfall_amount(value) for value in hourly.get('precipitation', [])],
        },
        'ml_analysis': {
    ...
    try:
        ...
        if start_offset < 0:
            ...
            for day in daily_forecast:
                ...
                rainfall.append(_normalize_rainfall_amount(day.get('rain_sum', day.get('precipitation_sum', 0))))
                # Use humidity from API
                if 'humidity_mean' in day:
                    humidity_val = day['humidity_mean']
                elif 'humidity_max' in day and 'humidity_min' in day:
            ...
            for i, day in enumerate(daily_forecast[start_offset:start_offset + num_days]):
                ...
                temps_max.append(day.get('temp_max', 0))
                temps_min.append(day.get('temp_min', 0))
                rainfall.append(_normalize_rainfall_amount(day.get('rain_sum', 0)))
    ...
    for i in range(len(daily_data['time'])):
        try:
            ...
            forecast_entry = {
                ...
                'precipitation_sum': _normalize_rainfall_amount(daily_data['precipitation_sum'][i]),
                'rain_sum': _normalize_rainfall_amount(daily_data.get('rain_sum', daily_data['precipitation_sum'])[i]),
                'humidity_mean': daily_data.get('relative_humidity_2m_mean', [65])[i],
